...
const AdminHomepage = () => {
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`${URL}/Therapist/${id}/reject`);
  ...
  const visibleTherapists = requestedTherapists.filter(
    (t) => !hiddenTherapists.has(t.id)
  );
  ...
  const currentTherapists = visibleTherapists.slice(
              ...
              <tr key={index}>
                ...
                <td>
                  <button
                    ...
                    onClick={() => handleAccept(therapist.id)}
                  >
                    Accept
                  </button>
                  <button
